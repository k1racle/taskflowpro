...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $step === 'install') {
    ...
    if (empty($user)) {
        ...
        try {
            ...
            // Создаём config.php
            createConfigFile($host, $user, $pass, $dbname, $licenseDomain);
            ...
            $message = 'Ошибка подключения к MySQL: ' . htmlspecialchars($e->getMessage());
            $messageType = 'error';
            $step = 'form';
        }
    }
...
function createConfigFile($host, $user, $pass, $dbname, $licenseDomain = ''): void {
    ...
    $replacements = [
        ...
        "define('JWT_SECRET'," => "define('JWT_SECRET', '{$jwtSecret}');",
        "define('APP_ENC_KEY'," => "define('APP_ENC_KEY', '{$appEncKey}');",
        "define('LICENSE_DOMAIN'," => "define('LICENSE_DOMAIN', '{$licenseDomainEsc}');",
    ];

    foreach ($replacements as $needle => $line) {
        if (preg_match("/" . preg_quote($needle, '/') . "[^;]*;/", $configContent)) {
            $configContent = preg_replace("/" . preg_quote($needle, '/') . "[^;]*;/", $line, $configContent, 1);
        } else {
            // если вдруг шаблона нет — просто добавим в начало после <?php
            $configContent = preg_replace('/^<\?php\s*/', "<?php\n" . $line . "\n", $configContent, 1);
        }
    }

    // JWT_EXPIRY оставляем как есть, либо добавим если отсутствует
    if (!preg_match("/define\('JWT_EXPIRY',/", $configContent)) {
